...
def main():
    ...
    p.add_option("-n", "--new", action="store_true", help="print debug output")
    ...
    def analyze_line(line, names, disp=False):
        ...
        m = re.search(
            r"([Tt]hanks to|[Cc]ourtesy of|Co-authored-by:) ([A-Z][A-Za-z]*? [A-Z][A-Za-z]*? [A-Z][A-Za-z]*|[A-Z][A-Za-z]*? [A-Z]\. [A-Z][A-Za-z]*|[A-Z][A-Za-z ]*? [A-Z][A-Za-z]*|[a-z0-9]+)($|\.| )",
            line,
        )
        if m:
            name = m.group(2)
...
class Cmd:
    ...
    def pipe(self, command, *a, **kw):
        ...
        p = self._call(
            command, a, {"stdin": stdin, "stdout": subprocess.PIPE}, call=False, **kw
        )
        return p.stdout

    def read(self, command, *a, **kw):
        p = self._call(command, a, {"stdout": subprocess.PIPE}, call=False, **kw)
