...
class EdfWriter:
    ...
    def setDatarecordDuration(self, record_duration: Union[float, int]) -> None:
        ...
        if (
            ...
            and abs(record_duration * 1e6 - round(record_duration * 1e6)) > 1e-6
        ):
            raise ValueError(
                "record_durations below 10 ms must be an integer multiple of 1 microsecond"
            )
    ...
    def blockWritePhysicalSamples(self, data: np.ndarray) -> int:
        ...
        return self._count_record(
            blockwrite_physical_samples(self.handle, data), block=True
        )

    def blockWriteDigitalSamples(self, data: np.ndarray) -> int:
        return self._count_record(
            blockwrite_digital_samples(self.handle, data), block=True
        )

    def blockWriteDigitalShortSamples(self, data: np.ndarray) -> int:
        return self._count_record(
            blockwrite_digital_short_samples(self.handle, data), block=True
        )
    ...
    def writeSamples(
        ...
        if self.buffered:
            ...
            self.sample_buffer = [
                np.ascontiguousarray(data_list[i][int(ind[i]) :])
