...
def check_is_ascii(string: str) -> None:
    ...
    if not all(ord(x) > 32 and ord(x) < 127 for x in string):
        warnings.warn(
            f'Invalid char: header entries should contain only ASCII characters and no spaces: "{string}"'
        )
...
def check_signal_header_correct(
    ...
    if len(str(ch["physical_min"])) > 8:
        warnings.warn(
            ...
            "some loss of precision is to be expected".format(
                i, label, ch["physical_min"], len(str(ch["physical_min"])), str(ch["physical_min"])[:8]
            )
        )
    if len(str(ch["physical_max"])) > 8:
        warnings.warn(
            ...
            "however, EDF+ can only save 8 chars, will be truncated to {}, "
            "some loss of precision is to be expected.".format(
                i, label, ch["physical_max"], len(str(ch["physical_max"])), str(ch["physical_max"])[:8]
            )
        )
...
def gender2int(gender: Union[int, str, None]) -> Optional[int]:
    warnings.warn(
        "Function 'gender2int' is deprecated, use 'sex2int' instead.", DeprecationWarning, stacklevel=2
    )
...
class EdfWriter:
    ...
    def __init__(
        ...
        file_name: str,
        n_channels: int,
        file_type: int = FILETYPE_EDFPLUS,
        buffered: bool = False,
        pad_with: Union[int, float, str] = 0,
    ) -> None:
        ...
        if isinstance(pad_with, str):
            if pad_with != "last":
                raise ValueError(f"pad_with must be a number or 'last', got {pad_with!r}")
        elif isinstance(pad_with, bool) or not isinstance(pad_with, numbers.Number):
            raise TypeError(f"pad_with must be a number or 'last', got {pad_with!r}")
    ...
    def update_header(self) -> None:
        ...
        patient_ident = (
            len(self.patient_code) + len(self.patient_name) + len(self.patient_additional) + 3 + 1 + 11
    ...
    def setGender(self, gender: Union[int, str, None]) -> None:
        warnings.warn(
            "Function 'setGender' is deprecated, use 'setSex' instead.", DeprecationWarning, stacklevel=2
        )
    ...
    def set_number_of_annotation_signals(self, number_of_annotations: int) -> None:
        ...
        if not 1 <= int(number_of_annotations) <= 64:
            warnings.warn(
                f"number_of_annotations must be between 1 and 64, "
    ...
    def writeSamples(self, data_list: Union[list[np.ndarray], np.ndarray], digital: bool = False) -> None:
        ...
        if (
            digital
            and not isinstance(self.pad_with, str)
            and not isinstance(self.pad_with, (int, np.integer))
        ):
            raise TypeError(f"Digital = True requires an integer pad_with, got {self.pad_with!r}")
        ...
        if self.buffered:
            if self._buffered_digital is None:
                ...
                raise TypeError(
                    "Cannot mix digital and physical samples in "
            ...
            if any(len(buf) > 0 for buf in self.sample_buffer):
                data_list = [
                    np.concatenate([buf, data]) if len(buf) > 0 else data
        ...
        for chan in self.channels:
            assert chan["physical_min"] != chan["physical_max"], (
                "In chan {} physical_min {} should be different from physical_max {}".format(
        ...
        while notAtEnd:
            ...
            for i in np.arange(len(data_list)):
                dataRecord = np.append(
                    dataRecord, data_list[i][int(ind[i]) : int(ind[i] + smp_per_record[i])]
                )
        ...
        if self.buffered:
            ...
            self.sample_buffer = [
                np.ascontiguousarray(data_list[i][int(ind[i]) :]) for i in range(len(data_list))
            ]
            return
    ...
    def writeAnnotation(
        ...
        if str_format == "utf_8":
            if duration_in_seconds >= 0:
                ...
                res = write_annotation_utf8(
                    self.handle, np.round(onset_in_seconds * 10000).astype(np.int64), -1, du(description)
                )
    ...
    def close(self) -> None:
        ...
        if self._n_records_written > 0 and self._n_annotations_written > capacity:
            ...
            if self.number_of_annotations < 64:
                advice = (
                    "Increase the number of annotation signals with "
                    "set_number_of_annotation_signals() before writing "
                    "any samples."
                )
            else:
                advice = (
                    "The maximum of 64 annotation signals is already "
                    "used, use a shorter record_duration to increase "
                    "the number of datarecords."
                )
            warnings.warn(
                f"File can only store {capacity} annotations "
