...
def _parse_date(string: str) -> datetime:
    ...
    """
    # some common formats.
    formats = ["%Y-%m-%d", "%d-%m-%Y", "%d.%m.%Y", "%Y.%m.%d", "%d %b %Y", "%Y/%m/%d", "%d/%m/%Y"]
    for f in formats:
        try:
...
def make_header(
    ...
    if gender is not None:
        if sex == "":
            sex = gender
            warnings.warn(
                "Parameter 'gender' is deprecated, use 'sex' instead.", DeprecationWarning, stacklevel=2
            )
        elif sex != gender:
            raise ValueError(
...
def write_edf(
    ...
    """
    assert header is None or isinstance(header, dict), "header must be dictioniary or None"
    ...
    for sig, shead in zip(signals, signal_headers):
        ...
        if digital:  # exception as it will lead to clipping
            assert dmin <= sig.min(), (
                f"digital_min is {dmin}, but signal_min is {sig.min()}for channel {label}"
            )
            assert dmax >= sig.max(), (
                f"digital_min is {dmax}, but signal_min is {sig.max()}for channel {label}"
            )
            assert pmin != pmax, f"physical_min {pmin} should be different from physical_max {pmax}"
            ...
            if abs(pmin - sig.min()) < edf_accuracy:
                warnings.warn(
                    f"phys_min is {pmin}, but signal_min is {sig.min()} for channel {label}",
            ...
            if abs(sig.max() - pmax) < edf_accuracy:
                warnings.warn(
                    f"phys_max is {pmax}, but signal_max is {sig.max()} for channel {label}",
                    category=UserWarning,
                )
            else:
                assert pmax >= sig.max(), (
...
def write_edf_quick(
    ...
    signal_headers = make_signal_headers(labels, sample_frequency=sfreq, physical_min=pmin, physical_max=pmax)
...
def compare_edf(
    ...
    for i, sigs in enumerate(zip(signals1, signals2)):
        ...
        assert close > 0.99, (
            "Error, digital values of {} and {} for ch {}: {} are not the same: {:.3f}".format(
    ...
    for i, sigs in enumerate(zip(signals1, signals2)):
        ...
        assert close > 0.99, (
            "Error, physical values of {} and {} for ch {}: {} are not the same: {:.3f}".format(
                edf_file1, edf_file2, shead1[i]["label"], shead2[i]["label"], close
            )
        )
    return True
...
def rename_channels(
    ...
    for ch_nr in tqdm(range(len(channels)), disable=not verbose):
        signal, signal_header, _ = read_edf(edf_file, digital=True, ch_nrs=ch_nr, verbose=verbose)
...
def _verify_polarity(
    ...
    for i, (s_old, s_new) in enumerate(zip(orig, new)):
        ...
        # of a whole unit
        step = abs(
            (shead["physical_max"] - shead["physical_min"]) / (shead["digital_max"] - shead["digital_min"])
        )
        if not np.allclose(s_new, expected, atol=step / 2, rtol=0):
            raise AssertionError(
                f"polarity of channel {i} ({shead['label']}) was not correctly inverted in {edf_target}"
            )
    ...
    for ch in channels:
        if isinstance(ch, str):
            if ch.lower() not in labels:
                raise ValueError(f"channel {ch} is not in {edf_file} (contains {labels})")
            to_invert.add(labels.index(ch.lower()))
        else:
            idx = len(labels) + ch if ch < 0 else ch
            if not 0 <= idx < len(labels):
                raise IndexError(f"channel index {ch} is out of range, {edf_file} has {len(labels)} channels")
            to_invert.add(idx)
