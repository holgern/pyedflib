...
class TestEdfWriter(unittest.TestCase):
    ...
    def test_set_number_of_annotation_signals(self):
        ...
        np.testing.assert_allclose(
            onsets[order], [round(i * 0.1, 1) for i in range(n_annotations)]
        )
        np.testing.assert_allclose(
            durations.astype(float)[order], [0.5] * n_annotations
        )
        self.assertEqual(
            [texts[i] for i in order], [f"annot_{i}" for i in range(n_annotations)]
        )
    ...
    def _buffered_ch_info(self, fs):
        return dict(
            self.ch_info_edf, sample_frequency=fs, physical_min=-5, physical_max=5
        )
