...
class TestHighLevel(unittest.TestCase):
    ...
    def test_write_more_annotations_than_datarecords(self):
        ...
        annotations = [
            [round(i * seconds / n_annotations, 4), -1, f"marker_{i}"] for i in range(n_annotations)
        ]
        header = highlevel.make_header()
        header["annotations"] = annotations
        ...
        np.testing.assert_allclose(
            [a[0] for a in read_annotations], sorted(a[0] for a in annotations), atol=1e-4
        )
    ...
    def test_assertion_dmindmax(self):
        ...
        edf_accuracy = min(
            [max(signals[0]) / sheaders[0]["digital_max"], min(signals[0]) / sheaders[0]["digital_min"]]
        ).astype(np.float16)
    ...
    def test_anonymize(self):
        ...
        highlevel.anonymize_edf(
            ...
            to_remove=["patientname", "birthdate", "admincode", "patientcode", "technician"],
    ...
    def test_crop_edf(self):
        ...
        _compare_cropped_edf(edf_file, outfile)
        # .. only start
        highlevel.crop_edf(edf_file, new_file=outfile, start=new_start_sec, start_format="seconds")
        _compare_cropped_edf(edf_file, outfile)
        # .. only stop
    ...
    def test_drop_channel(self):
        ...
        highlevel.drop_channels(self.drop_from, self.drop_from[:-4] + "2.edf", to_drop=["ch0", "ch1", "ch2"])
    ...
    def test_change_polarity(self):
        ...
        signal_headers = highlevel.make_signal_headers(
            ["ch0", "ch1", "ch2"], sample_frequency=100, physical_min=-200, physical_max=200
        )
        ...
        def inverted_channels(file):
            ...
            return {i for i in range(len(new)) if np.allclose(new[i], -orig[i], atol=step / 2, rtol=0)}
        ...
        for channels, expected in [
            ...
            assert highlevel.change_polarity(edf_file, channels, new_file=outfile)
        ...
        with self.assertRaises(ValueError):
            highlevel.change_polarity(edf_file, ["not_a_channel"], new_file=outfile)
    ...
    def test_change_polarity_asymmetric_range(self):
        ...
        signal_headers = highlevel.make_signal_headers(
            ["spo2"], dimension="%", sample_frequency=100, physical_min=0, physical_max=100
        )
        ...
        assert highlevel.change_polarity(edf_file, ["spo2"], new_file=outfile, verify=True)
